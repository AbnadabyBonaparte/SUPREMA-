        ...
        <div className="py-10 max-w-4xl mx-auto">
            ...
            <div className="flex justify-between items-center mb-10">
                <div>
                    ...
                    <p className="text-muted">Gerencie suas informações pessoais e preferências</p>
                </div>
                {!isEditing && (
                    <Button
                        onClick={() => setIsEditing(true)}
                        className="bg-primary text-foreground-inverse hover:bg-primary-hover"
                    >
                        Editar Perfil
                    </Button>
                )}
            ...
            <div className="grid lg:grid-cols-3 gap-8">
                ...
                <div className="space-y-6">
                    <Card className="bg-surface border-border p-6 text-center">
                        ...
                        <h3 className="text-foreground font-bold text-xl mb-1">{formData.name}</h3>
                        <p className="text-muted text-sm mb-4">{formData.email}</p>

                        {isEditing && (
                            <Button variant="outline" className="w-full border-primary text-primary hover:bg-primary hover:text-foreground-inverse text-sm">
                                Alterar Foto
                            </Button>
                        )}
                    </Card>
                    ...
                    <Card className="bg-[#1A1A1A] border-[#333] p-6">
                        <h4 className="text-foreground font-bold mb-4">Estatísticas</h4>
                        <div className="space-y-3">
                            <div className="flex justify-between">
                                <span className="text-muted text-sm">Membro desde</span>
                                <span className="text-foreground text-sm font-medium">Jan 2024</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-muted text-sm">Agendamentos</span>
                                <span className="text-foreground text-sm font-medium">24</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-muted text-sm">Consultorias IA</span>
                                <span className="text-foreground text-sm font-medium">47</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-muted text-sm">Nível</span>
                                <span className="text-primary text-sm font-bold">Black Member</span>
                            </div>
                        </div>
                    </Card>
                </div>
                            ...
                            <div className="grid md:grid-cols-2 gap-5">
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full px-4 py-3 bg-background border border-border text-foreground rounded-lg focus:outline-none focus:border-primary/50 disabled:opacity-50"
                                    />
                                </div>
                    ...
                    {isEditing && (
                        <div className="flex gap-4">
                            ...
                            <Button
                                onClick={handleSave}
                                disabled={isSaving}
                                className="flex-1 bg-primary text-foreground-inverse hover:bg-primary-hover"
                            >
